...
export  class BrowserTab extends Tab {
    ...
    async 查找反向链接(){ 
        ...
        let {BlockListEditor} = await   import("../blockList/index.js")
        this.backlinkListElement.innerHTML = ''
        let blocks =  await 核心api.fullTextSearchBlock({
            path:'',
            query:this.urlInputter.value,
        ...
        let blocks1 =  await 核心api.fullTextSearchBlock({
            path:'',
            query:this.headElement.querySelector(`span.item__text`).innerText,
            "types":{"document":true,"heading":true,"list":true,"listItem":true,"codeBlock":true,"htmlBlock":true,"mathBlock":true,"table":true,"blockquote":true,"superBlock":true,"paragraph":true}
        })
        let list =   (blocks.blocks).concat(blocks1.blocks)
        new BlockListEditor(this.backlinkListElement,list)
    }
}
