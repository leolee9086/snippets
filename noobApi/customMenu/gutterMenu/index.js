        ...
        switch (判定元素.tagName) {
            ...
            case 'DIV':
                if (判定元素.classList && 判定元素.classList.contains('protyle-gutters')) {
                    菜单状态.当前块id = 判定元素.querySelector('BUTTON').getAttribute('data-node-id')
                    菜单状态.当前块类型 = 判定元素.querySelector('BUTTON').getAttribute('data-type')
                    菜单状态.当前子块类型 = 判定元素.querySelector('BUTTON').getAttribute('data-subType')
                }                    
