...
export default function 注册工具栏() {
  ...
  function 生成搜索结果条目(结果条目, 当前块id) {
    ...
    let 事件配置 = {
      mousemove: (e) => {
        e.currentTarget.classList.add("b3-menu__item--current");
      },
      mouseleave: (e) => {
        e.currentTarget.classList.remove("b3-menu__item--current");
      },
      click: () => {
        window.siyuan.ctrlIsPressed
          ? 根据目标id移动块所在文档(当前块id, 结果条目.id)
  ...
  function 生成搜索结果子条目(结果条目) {
    ...
    <svg style="padding-left: 22px;"  class="b3-list-item__graphic popover__block" data-id="${
      结果条目.id
    }">
        <use xlink:href="#${getIconByType(
          结果条目.type,
          结果条目.subtype
        )}"></use>
    </svg>
    <span class="b3-list-item__text" style="color:var(--b3-theme-on-surface)">${
      结果条目.content
    }</span>
    </div>
    `;
  ...
  async function 显示目标选择面板(event) {
    ...
    let 搜索结果 = await noobApi.核心api.全文搜索块({
      query: 获取工具栏对应range(工具栏元素).toString(),
    });
    console.log(搜索结果);
    let 面板元素 = 工具栏元素.parentElement.querySelector(".protyle-hint");
    面板元素.innerHTML = "";
    let 当前块id =
      获取工具栏对应块元素(工具栏元素).getAttribute("data-node-id");
    搜索结果.blocks.forEach((结果条目) => {
      if (!面板元素.querySelector(`[data-item-id="${结果条目.rootID}"]`)) {
        let 条目元素 = 生成搜索结果条目(结果条目, 当前块id);
        面板元素.appendChild(条目元素);
        面板元素.insertAdjacentHTML(
          "beforeEnd",
          `<button class="b3-menu__separator"></button>`
        );
      } else {
        面板元素
          .querySelector(`[data-item-id="${结果条目.rootID}"]`)
          .appendChild(生成搜索结果子条目(结果条目));
      }
    });
